...
def update_credential_provider_path(
  config, config_type, dest_provider_path, file_owner, file_group, use_local_jceks=False
):
  """
  ...
  if HADOOP_CREDENTIAL_PROVIDER_PROPERTY_NAME in config:
    ...
    if use_local_jceks:
      config_copy[HADOOP_CREDENTIAL_PROVIDER_PROPERTY_NAME] = (
        f"localjceks://file{dest_provider_path}"
      )
    else:
      config_copy[HADOOP_CREDENTIAL_PROVIDER_PROPERTY_NAME] = (
        f"jceks://file{dest_provider_path}"
      )
    return config_copy
  return config
      ...
      if rules:
        for property_name, expected_value in rules.items():
          ...
          if actual_value != expected_value:
            issues[config_file] = (
              "Property %s contains an unexpected value. "
              "Expected/Actual: %s/%s" % (property_name, expected_value, actual_value)
            )
...
def cached_kinit_executor(
  kinit_path, exec_user, keytab_file, principal, hostname, temp_dir, expiration_time=5
):
  """
  ...
  if (
    ...
    or ("last_successful_execution" not in output[key])
  ):
    new_cached_exec(
      key, file_path, kinit_path, temp_dir, exec_user, keytab_file, principal, hostname
    )
  else:
