    ...
    with tempfile.NamedTemporaryFile() as tmpf:
      with tempfile.NamedTemporaryFile() as old_repo_tmpf:
        for (
          repo_file_path,
          repo_file_content,
        ) in RepositoryProvider.repo_files_content.items():
...
class UbuntuRepositoryProvider(RepositoryProvider):
  ...
  update_cmd = [
    ...
    "-qq",
    "-o",
    "Dir::Etc::sourcelist=sources.list.d/{repo_file_name}",
    "-o",
    "Dir::Etc::sourceparts=-",
    "-o",
  ...
  def action_prepare(self):
    repo_file_name = format(
      "{repo_file_name}.list", repo_file_name=self.resource.repo_file_name
    )
