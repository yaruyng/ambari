...
def run_powershell_script(script_content):
  ...
  result = os_run_os_command(
    f"powershell  -ExecutionPolicy unrestricted -File {script_file.name}"
  )
...
def os_is_root():
  ...
  if retcode != 0:
    err_msg = (
      f"Unable to check the current user's group memberships. "
...
class WinService(win32serviceutil.ServiceFramework):
  ...
  def _StopOrWaitForChildProcessToFinish(self, childProcess):
    ...
    if win32event.WAIT_OBJECT_0 == win32event.WaitForMultipleObjects(
      [WinService._heventSvcStop, childProcess._handle], False, win32event.INFINITE
    ):
...
class UserHelper(object):
  ...
  def parse_user_name(userName, defDomain=None):
    ...
    if domainSepIndex != -1:
      ...
      if (
        not domainName or domainName == "." or domainName == win32api.GetComputerName()
      ):
        domainName = defDomain
