...
KRB5_REALM_PROPERTIES = ["kdc", "admin_server", "default_domain", "master_kdc"]
...
class MissingKeytabs(object):
  ...
  def from_kerberos_records(self, kerberos_record, hostname):
    if kerberos_record is not None:
      with_missing_keytab = (
        each
        for each in kerberos_record
        if not self.keytab_exists(each) or not self.keytab_has_principal(each, hostname)
      )
      return MissingKeytabs(
        set(
          MissingKeytabs.Identity.from_kerberos_record(each, hostname)
          for each in with_missing_keytab
        )
      )
    else:
  ...
  def keytab_has_principal(kerberos_record, hostname):
    principal = get_property_value(kerberos_record, "principal").replace(
      "_HOST", hostname
    )
  ...
  def __str__(self):
    return (
      ...
      else "No missing keytabs"
    )


def write_krb5_conf(params):
  Directory(
    params.krb5_conf_dir, owner="root", create_parents=True, group="root", mode=0o755
  )

  content = InlineTemplate(params.krb5_conf_template)

  File(params.krb5_conf_path, content=content, owner="root", group="root", mode=0o644)
...
def find_missing_keytabs(params, output_hook=lambda missing_keytabs: None):
  missing_keytabs = MissingKeytabs.from_kerberos_records(
    params.kerberos_command_params, params.hostname
  )
...
def resolve_encryption_family_list(enc_types_list):
  ...
  for each in enc_types_list:
    result.extend(
      ENCRYPTION_FAMILY_MAP[each] if each in ENCRYPTION_FAMILY_MAP else [each]
    )
  return set(result)
