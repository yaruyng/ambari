...
def setup_historyserver():
  ...
  params.HdfsResource(
    "/mapred", type="directory", action="create_on_execute", owner=params.mapred_user
  )
  params.HdfsResource(
    "/mapred/system",
    type="directory",
...
def setup_nodemanager():
  ...
  if not params.security_enabled or params.toggle_nm_security:
    ...
    nm_log_dir_to_mount_file_content = handle_mounted_dirs(
      create_log_dir, params.nm_log_dirs, params.nm_log_dir_to_mount_file, params
    )
    ...
    nm_local_dir_to_mount_file_content = handle_mounted_dirs(
      create_local_dir, params.nm_local_dirs, params.nm_local_dir_to_mount_file, params
    )
    File(
...
def setup_resourcemanager():
  ...
  if (
    not is_empty(params.node_label_enable)
    and params.node_label_enable
    or is_empty(params.node_label_enable)
    and params.node_labels_dir
  ):
    params.HdfsResource(
...
def setup_atsv2_hbase_files():
  ...
  File(
    format("{yarn_hbase_grant_premissions_file}"),
    owner=params.yarn_hbase_user,
    group=params.user_group,
    mode=0o644,
...
def setup_atsv2_hbase_directories():
  ...
  Directory(
    [
      ...
      params.yarn_hbase_pid_dir,
      params.yarn_hbase_log_dir,
    ],
    owner=params.yarn_hbase_user,
    group=params.user_group,
    create_parents=True,
...
def setup_system_services(config_dir=None):
  ...
  user_dir = format(
    "{yarn_system_service_dir}/{yarn_system_service_launch_mode}/{yarn_hbase_user}"
  )
  params.HdfsResource(
    user_dir,
    type="directory",
  ...
  params.HdfsResource(
    format("{user_dir}/hbase.yarnfile"),
    type="file",
    action="create_on_execute",
  ...
  params.HdfsResource(
    format("{yarn_hbase_user_version_home}"),
    type="directory",
    action="create_on_execute",
    owner=params.yarn_hbase_user,
    group=params.user_group,
  ...
  params.HdfsResource(
    format("{yarn_hbase_user_version_home}/core-site.xml"),
    type="file",
    action="create_on_execute",
  ...
  params.HdfsResource(
    format("{yarn_hbase_user_version_home}/hbase-site.xml"),
    type="file",
    action="create_on_execute",
  ...
  params.HdfsResource(
    format("{yarn_hbase_user_version_home}/hbase-policy.xml"),
    type="file",
    action="create_on_execute",
  ...
  params.HdfsResource(
    format("{yarn_hbase_user_version_home}/log4j.properties"),
    type="file",
    action="create_on_execute",
  ...
  if params.has_metric_collector:
    params.HdfsResource(
      format("{yarn_hbase_user_version_home}/hadoop-metrics2-hbase.properties"),
      type="file",
      action="create_on_execute",
  ...
  params.HdfsResource(
    format("{yarn_service_app_hdfs_path}"),
    type="directory",
    action="create_on_execute",
    owner=params.hdfs_user,
