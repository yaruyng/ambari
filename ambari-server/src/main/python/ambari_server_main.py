...
IS_FOREGROUND = (
  ENV_FOREGROUND_KEY in os.environ and os.environ[ENV_FOREGROUND_KEY].lower() == "true"
)
...
AMBARI_SERVER_DIE_MSG = (
  "Ambari Server java process died with exitcode {0}. Check {1} for more information."
)
AMBARI_SERVER_NOT_STARTED_MSG = (
  "Ambari Server java process hasn't been started or can't be determined."
)
AMBARI_SERVER_STOPPED = (
  "Ambari Server java process has stopped. Please check the logs for more information."
)
AMBARI_SERVER_UI_TIMEOUT = (
  "Server not yet listening on http port {0} after {1} seconds. Exiting."
)
...
def get_ulimit_open_files(properties):
  ...
  open_files = (
    int(open_files_val)
    if (open_files_val and int(open_files_val) > 0)
...
def generate_child_process_param_list(
  ...
  if is_root() and ambari_user != "root":
    ...
    cmd = "{ulimit_cmd} ; {su} {ambari_user} -s {sh_shell} -c '. {ambari_env_file} && {command}'".format(
...
def wait_for_server_start(pidFile, scmStatus):
  ...
  if pid:
    ...
    waitStart = time.time()
    if not wait_for_ui_start(
      int(ambari_server_ui_port), pid, web_server_startup_timeout
    ):
    ...
    exception = FatalException(
      -1, AMBARI_SERVER_DIE_MSG.format(exitcode, configDefaults.SERVER_OUT_FILE)
    )
  else:
...
def server_process_main(options, scmStatus=None):
  ...
  if jdk_path is None:
    err = (
      'No JDK found, please run the "ambari-server setup" '
  ...
  if not options.skip_properties_validation:
    ...
    if missing_properties:
      err = (
        "Required properties are not found: "
        + str(missing_properties)
        + ". To skip properties validation "
  ...
  if options.skip_database_check:
    ...
    print(
      "Ambari Server is starting with the database consistency check skipped. Do not make any changes to your cluster "
  ...
  def make_process_independent():
    if (
      IS_FOREGROUND
    ):  # upstart script is not able to track process from different pgid.
      return
  ...
  procJava = subprocess.Popen(
    param_list, env=environ, preexec_fn=make_process_independent
  )
