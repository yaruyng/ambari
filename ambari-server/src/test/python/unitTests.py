...
def get_extends_field_from_metainfo(service_metainfo):
  ...
  stack_metainfo = os.path.join(
    os.path.dirname(service_metainfo), "..", "..", "metainfo.xml"
  )
...
def resolve_paths_to_import_from_common_services(
  metainfo_file, base_stack_folder, common_services_parent_dir, service
):
  """
  ...
  if extract is not None:
    if "common-services" in extract:
      ...
      scripts_path = os.path.join(
        common_services_parent_dir, extract, "package", "scripts"
      )
      ...
      metainfo_file = os.path.join(
        inherited_from_older_version_path, "services", service, "metainfo.xml"
      )
      if os.path.isdir(inherited_from_older_version_path):
        paths_to_import += resolve_paths_to_import_from_common_services(
          metainfo_file,
          inherited_from_older_version_path,
          common_services_parent_dir,
          service,
        )
  else:
    print(
      f"Service {service}. Could not get extract <extends></extends> from metainfo file: {str(metainfo_file)}. This may prevent modules from being imported."
    )
...
def append_paths(server_src_dir, base_stack_folder, service):
  ...
  if base_stack_folder is not None:
    ...
    if base_stack_folder is not None:
      metainfo_file = os.path.join(
        base_stack_folder, "services", service, "metainfo.xml"
      )

    common_services_parent_dir = os.path.join(server_src_dir, "main", "resources")
    paths_to_add = resolve_paths_to_import_from_common_services(
      metainfo_file, base_stack_folder, common_services_parent_dir, service
    )
  else:
    ...
    base_common_services_folder = os.path.join(
      server_src_dir, "main", "resources", "common-services"
    )
    for folder, subFolders, files in os.walk(
      os.path.join(base_common_services_folder, service)
    ):
...
def stack_test_executor(base_folder, service, stack, test_mask, executor_result):
  ...
  if stack is not None:
    base_stack_folder = os.path.join(
      server_src_dir, "main", "resources", "stacks", get_stack_name(), stack
    )

    for root, subFolders, files in os.walk(
      os.path.join(base_stack_folder, "services", service)
    ):
...
def main():
  ...
  ):  # handle custom_tests for backward-compatibility
  ...
  ambari_agent_folder = os.path.normpath(
    os.path.join(ambari_server_folder, "../ambari-agent")
  )
  ambari_common_folder = os.path.normpath(
  ...
  sys.path.append(
    os.path.join(ambari_server_folder, "src/main/resources/custom_actions/scripts")
  )
          ...
          if service == "hooks":
            for hook in os.listdir(current_service_dir):
              test_variants.append(
                {
                  "directory": os.path.join(current_service_dir, hook),
                  "service": hook,
                  "stack": stack,
                }
              )
            ...
            test_variants.append(
              {"directory": current_service_dir, "service": service, "stack": stack}
            )
