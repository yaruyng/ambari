...
class ScriptAlert(BaseAlert):
  ...
  def _collect(self):
    ...
    if cmd_module is not None:
      ...
      try:
        ...
        logger.debug(
          f"The script {str(cmd_module)} does not have a get_tokens() function"
        )
      ...
      if matchObj:
        ...
        with Environment(
          basedir, tmp_dir=AGENT_TMP_DIR, logger=logging.getLogger("alerts")
        ) as env:
  ...
  def _load_source(self):
    ...
    if not os.path.exists(self.path_to_script) or not os.path.isfile(
      self.path_to_script
    ):
      raise Exception(
        "Unable to find '{0}' as an absolute path or part of {1} or {2}".format(
